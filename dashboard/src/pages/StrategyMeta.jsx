                                ...
                                <div className="space-y-6">
                                    {STATIC_SLOTS.map((slot) => (
                                        <div key={slot.id} className="bg-gray-900 p-5 rounded-xl border border-gray-700">
                                                ...
                                                {(schedule[slot.id] || []).map(fid => {
                                                    ...
                                                    return (
                                                        <div key={cap.fid} className="flex items-center gap-2 p-2 rounded-lg bg-gray-800 border border-gray-600 text-gray-200 relative group pr-8">
                                                            ...
                                                            <button onClick={() => removeCaptainFromSlot(slot.id, cap.fid)} className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 transition-opacity p-1 bg-gray-900 rounded-full">
                                                                <X size={14} />
                                                            </button>
                                                        </div>
                                                    );
                                            ...
                                            <select value="" onChange={(e) => addCaptainToSlot(slot.id, e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-400 outline-none focus:border-yellow-500 w-full md:w-auto">
