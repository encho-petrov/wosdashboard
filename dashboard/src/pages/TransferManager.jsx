                                                    ...
                                                    return (
                                                        <tr key={r.id} className={`border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors ${
                                                            ...
                                                            <td className="p-4 text-right font-black pr-6 text-[10px] uppercase">
                                                                {isConfirmed && r.direction === 'Inbound' && <span className="text-green-500 flex justify-end items-center gap-1.5"><Shield size={12}/> Confirmed</span>}
                                                                {isConfirmed && r.direction === 'Outbound' && <span className="text-red-500 flex justify-end items-center gap-1.5"><LogOut size={12}/> Offboarded</span>}
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex flex-col items-end gap-1.5">
                                                            ...
                                                            <div className="flex items-center gap-1.5">
                                                                ...
                                                                {isConfirmed && r.direction === 'Inbound' && <Shield size={14} className="text-green-500 ml-1" />}
                                                                {isConfirmed && r.direction === 'Outbound' && <LogOut size={14} className="text-red-500 ml-1" />}
                                ...
                                return (
                                    <tr key={r.id} className={`border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors ${
                                        ...
                                        isDeclined ? 'bg-red-900/5 opacity-60' : ''
                                            ...
                                            {isOutbound && <span className="px-2 py-0.5 bg-red-900/40 text-red-400 text-[9px] rounded-lg uppercase font-black border border-red-800/50 tracking-widest">Exiting</span>}
                                        </td>
                                        ...
                                        <td className="p-4">
                                            {isOutbound ? (
                                                <span className="text-gray-400 font-bold tracking-widest text-[10px]">
                                                    {alliances.find(a => a.id === r.targetAllianceId)?.name || 'Unknown'}
                                                </span>
                                            ) : (
                                                <select
                                        ...
                                        <td className="p-4">
                                            {isOutbound ? (
                                                <span className="text-yellow-500 font-mono text-[11px] font-bold">{r.power.toLocaleString()}</span>
                                            ) : (
                                        ...
                                        <td className="p-4">
                                            {isOutbound ? (
                                                <div className="text-center text-gray-600 text-[10px] font-black uppercase">-</div>
                                            ) : (
                                        ...
                                        <td className="p-4 text-right pr-6">
                                            <div className="flex justify-end gap-2">
                                                {r.status === 'Pending' && isAdmin && !isOutbound ? (
                                                    <>
                                                        <button
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {isConfirmed && !isOutbound && <span className="text-green-500 text-[10px] font-black uppercase flex items-center gap-2"><Shield size={14}/> Onboarded</span>}
                                                        {isConfirmed && isOutbound && <span className="text-red-500 text-[10px] font-black uppercase flex items-center gap-2"><LogOut size={14}/> Offboarded</span>}
                        ...
                        return (
                            <div
                                ...
                                className={`border rounded-xl p-4 flex items-center justify-between transition-all cursor-pointer active:scale-[0.98] ${
                                    isNormal && !isOutbound ? 'border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.15)]' :
                                    isSpecial && !isOutbound ? 'border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.15)]' :
                                    isConfirmed && !isOutbound ? 'border-green-800/50 shadow-md' :
                                    isConfirmed && isOutbound ? 'border-red-800/50 shadow-md' :
                                    isDeclined ? 'border-red-800/50 shadow-md' : 'border-gray-800 shadow-md'
                                } ${
                                    isConfirmed && !isOutbound ? 'bg-green-900/10' :
                                    isConfirmed && isOutbound ? 'bg-red-900/10 opacity-80' :
                                    isDeclined ? 'bg-red-900/10 opacity-60' : 'bg-gray-900'
                                }`}
                            >
                                <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col items-end gap-1.5">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        {/* Glowing Dots */}
                                        {isNormal && !isOutbound && <span className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.8)]"></span>}
                                        {isSpecial && !isOutbound && <span className="w-2 h-2 rounded-full bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.8)]"></span>}

                                        {/* Status Icons */}
                                        {isConfirmed && !isOutbound && <Check size={16} className="text-green-500 ml-1" />}
                                        {isConfirmed && isOutbound && <LogOut size={16} className="text-red-500 ml-1" />}
                ...
                <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[60] p-4">
                    <div className="bg-gray-900 p-6 rounded-3xl w-full max-w-sm border border-gray-700 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="space-y-5">
                            <div>
                                <label className="text-[10px] font-black tracking-widest text-gray-500 block mb-1">
                                    {activeMobileRecord.direction === 'Outbound' ? 'Original Alliance' : 'Destination Alliance'}
                                </label>
                                {activeMobileRecord.direction === 'Outbound' ? (
                                    <div className="w-full bg-gray-950 border border-gray-800 rounded-xl p-3 text-xs font-bold text-gray-400">
                                        {alliances.find(a => a.id === activeMobileRecord.targetAllianceId)?.name || 'Unknown Alliance'}
                                    </div>
                                ) : (
                                    <select
                            ...
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1">
                                    {activeMobileRecord.direction === 'Outbound' ? 'Power Record' : 'Update Power'}
                                </label>
                                {activeMobileRecord.direction === 'Outbound' ? (
