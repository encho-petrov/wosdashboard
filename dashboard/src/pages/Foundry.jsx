                ...
                {!isBench && (
                    <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                        {isHistory ? (
                            ...
                            <button aria-label={`Remove ${p.nickname}`} onClick={() => handleDeploy(pFid, null, false)} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors shrink-0">
                                <X size={14} />
                            </button>
                        ) : isLocked && isAdmin ? (
                            <select
            ...
            <div className={`bg-gray-900 rounded-3xl border transition-all flex flex-col overflow-hidden ${isLocked && !isHistory ? 'border-red-900/50 bg-red-950/5' : 'border-gray-800 shadow-2xl'}`}>
                <div className={`p-4 flex justify-between items-center border-b ${isLocked && !isHistory ? 'bg-red-900/10 border-red-900/20' : 'bg-gray-900/50 border-gray-800'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {isAdmin && !isHistory && (
                            <button aria-label={isLocked ? `Unlock Legion ${legionId}` : `Lock Legion ${legionId}`} onClick={() => toggleLock(legionId, isLocked)} className={`p-2 rounded-xl border transition-all ${isLocked ? 'bg-red-500 text-white border-red-400 shadow-lg shadow-red-500/20' : 'bg-gray-800 text-gray-500 hover:text-white border-gray-700'}`}>
