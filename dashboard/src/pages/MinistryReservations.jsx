...
export default function MinistryReservations() {
    ...
    //if (loading || globalLoading) return <div className="p-10 text-white font-mono">LOADING MINISTRY DATA...</div>;
                    ...
                    {showCreateModal && (
                        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
                            <div className="bg-gray-800 p-6 rounded-lg w-[500px] border border-gray-700 shadow-2xl">
                                ...
                                <input type="date" className="w-full mb-4 p-2 bg-gray-900 border border-gray-700 rounded text-white outline-none focus:border-blue-500"
                                       value={formBaseDate} onChange={handleBaseDateChange} />
                                <input type="text" placeholder="Event Title" className="w-full mb-4 p-2 bg-gray-900 border border-gray-700 rounded text-white font-bold outline-none focus:border-blue-500"
    ...
    return (
        <AdminLayout title={pageTitle} actions={ministryActions}>
            <div className="p-6 h-full flex flex-col min-h-0 bg-gray-900">
                ...
                {!viewingHistory && event && (
                    <div className="flex gap-4 mb-4 shrink-0">
                        ...
                        <button
                            ...
                            disabled={!isAdmin}
                            className={`px-4 py-2 flex items-center gap-3 text-xs font-black uppercase tracking-widest rounded-xl border transition-all ${
                ...
                <div className="flex-1 flex gap-6 min-h-0">
                    {viewingHistory && (
                        <div className="w-64 bg-gray-800/50 border border-gray-700 rounded-xl overflow-y-auto p-3 flex flex-col gap-2 shrink-0 custom-scrollbar shadow-lg">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6 h-full min-h-0">
                        {schedule.map(day => (
                            <div key={day.id} className="bg-gray-800 border border-gray-700 rounded-xl flex flex-col h-full overflow-hidden shadow-2xl">
                                ...
                                <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar bg-black/10">
                                    {day.slots.map(slot => (
                                        <div key={slot.id} className={`flex items-center p-2 rounded-lg border transition-all ${
                                            ...
                                            <div className="w-20 shrink-0 font-mono text-[10px] font-black text-gray-500 flex items-center gap-1.5">
                                                ...
                                                {slot.playerFid ? (
                                                    <>
                                                        <div className="truncate">
                                                            <div className="font-bold text-gray-100 text-sm truncate">{slot.nickname}</div>
                                                            <div className="text-[9px] text-gray-500 font-black uppercase truncate tracking-tighter">
                                                                {slot.allianceName || 'Independent'} {viewingHistory ? '' : `| ${slot.playerFid}`}
                                                            </div>
                                                        </div>
                                                        {!viewingHistory && (
                                                            <button onClick={() => handleClearSlot(slot)} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all">
                                                                <X size={14} />
                                                            </button>
                                                        )}
                                                    </>
                                                    ...
                                                    ) : <span className="text-[10px] text-gray-700 italic font-black uppercase">Vacant</span>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                ...
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 p-6 rounded-2xl w-full max-w-md border border-gray-700 shadow-2xl">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                <h3 className="text-xl font-black text-white tracking-tighter">SELECT OPERATOR</h3>
                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Slot: {formatTimeUTC(searchModal.slot?.slotIndex)} UTC</p>
                            </div>
                            <button onClick={() => setSearchModal({ isOpen: false, slot: null, query: '' })} className="p-2 text-gray-500 hover:text-white bg-gray-900 rounded-xl transition-colors">
                                <X size={20}/>
                            </button>
                        </div>
                        ...
                        <div className="relative mb-6">
                            ...
                            <input
                                type="text" autoFocus
                                placeholder="Search Name or FID..."
                                className="w-full pl-10 p-3 bg-gray-900 border border-gray-700 rounded-xl text-white outline-none focus:border-blue-500 transition-all shadow-inner"
                        ...
                        <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                            {filteredRoster.length === 0 ? (
                                <div className="text-center py-10">
                                    <Search className="w-12 h-12 text-gray-700 mx-auto mb-2 opacity-20" />
                                    <p className="text-gray-600 text-xs font-bold uppercase">No records found</p>
                                </div>
                            ) : (
                                filteredRoster.slice(0, 50).map(p => (
                                    <button
                                        ...
                                        className="w-full flex justify-between items-center p-3 bg-gray-900/50 hover:bg-blue-600/20 border border-gray-800 hover:border-blue-500/50 rounded-xl transition-all text-left group"
                                    >
                                        <div>
                                            <div className="font-bold text-sm text-gray-200 group-hover:text-white">{p.nickname}</div>
                                            <div className="text-[10px] text-gray-500 font-mono">{p.fid}</div>
                                        </div>
                                        <div className="text-[9px] font-black uppercase tracking-widest bg-gray-800 px-2.5 py-1 rounded-lg border border-gray-700 text-gray-400">
