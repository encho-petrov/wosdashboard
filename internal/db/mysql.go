...
func (s *Store) CloseTransferSeason(seasonID int) error {
	_, err := s.db.Exec("UPDATE transfer_seasons SET status = 'Closed', closed_at = NOW() WHERE id = ?", seasonID)
	return err
}
...
func (s *Store) ConfirmOutboundTransfer(fid int64, seasonID int, nickname string, destState string) error {
	...
	insertQuery := `
        INSERT INTO transfer_records (season_id, fid, direction, nickname, source_state, status) 
        VALUES (?, ?, 'Outbound', ?, ?, 'Confirmed')`
	if _, err = tx.Exec(insertQuery, seasonID, fid, nickname, destState); err != nil {
		return err
	}
